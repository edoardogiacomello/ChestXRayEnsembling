...
class VAE(tf.keras.Model):
    def __init__(self, number_of_classes=2, dummy_model=True, latent_dim=100):
        ...
        if self.num_classes == 2:
            self.bottleneck_dim = 2 * self.latent_dim + 1
        elif self.num_classes > 2:
            self.bottleneck_dim = 2 * self.latent_dim + self.num_classes
        elif self.num_classes < 2 or (self.num_classes is None):
            self.bottleneck_dim = 2 * self.latent_dim
        ...
        if dummy_model:
            self.encoder = ff.cnn_to_encoder(
                ...
                latent_space_dim=self.bottleneck_dim
            )
            self.decoder = ff.make_std_decoder(
                conv_steps=3,
                final_images_size=(128, 128))

        print('self.latent_dim : {}'.format(self.latent_dim))
        print('self.num_classes : {}'.format(self.num_classes))
        print('self.bottleneck : {}'.format(self.bottleneck_dim))

    def summary(self, expected_dims:list=[128, 128, 3]):
        print('Encoder : ')
    ...
    def load_model_from_file(self, encoder_path='models/encoder', decoder_path='models/decoder'):
    ...
    def load_encoder(self, model, is_encoder=False, keep_last_layer=True):
        """
        Load an encoder model from a pre-existing model, i.e. tf.keras.application.vgg19
        :param model: model to be used as encoder
        :param is_encoder: specify if the model has the correct output already, otherwise it is attached
        :param keep_last_layer: specify if the last layer has to be kept or substituted with the intended one
        :return: None
        """
        self.encoder = model if is_encoder else ff.cnn_to_encoder(model, latent_space_dim=self.bottleneck_dim, keep_last=keep_last_layer)

    def load_decoder(self, model: tf.keras.Model):
        """
        Load a decoder model from a pre-existing model
        :param model: model to be used as decoder
        :return: None
        """
        self.decoder = model
    ...
    def vae_loss_function(true_img, recon_img, mu, logsigma, kl_weight=0.0005):
        ...
        reconstruction_loss = tf.reduce_mean(tf.abs(true_img - recon_img), axis=(1, 2, 3))
